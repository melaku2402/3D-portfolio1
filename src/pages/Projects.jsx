...
// For technologies without specific icons, we'll use generic ones
import { 
  FaMobileAlt,
  FaDatabase,
  FaServer,
...
const tagIconMap = {
  ...
  stripe: SiStripe,
  firebase: SiFirebase,
  aws: SiAmazon,  // Fixed: Changed from SiAmazonaws to SiAmazon
  pwa: SiPwa,
  ...
  // Fallbacks for common technologies
...
const ProjectCard = ({
  ...
  const getTagIcon = (tagName) => {
    ...
    const IconComponent = tagIconMap[lowerTagName];
    return IconComponent ? <IconComponent className="w-4 h-4" /> : <FaCode className="w-4 h-4" />;
  };
        ...
        <div className="relative w-full h-48 sm:h-56 md:h-64 overflow-hidden rounded-xl">
          ...
          <div className="absolute inset-0 flex justify-end m-3">
            <div
              ...
              className="bg-gradient-to-br from-[#1a1a2e] to-[#16213e] w-8 h-8 sm:w-10 sm:h-10 rounded-full flex justify-center items-center cursor-pointer transition-all duration-300 hover:scale-110 border border-gray-700 shadow-lg"
            >
              <FiGithub className="w-4/5 h-4/5 text-white" />
            </div>
          </div>
        ...
        <div className="p-4 sm:p-6 flex-1 flex flex-col">
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            {tags.map((tag) => (
              <span
                key={`${name}-${tag.name}`}
                className={`text-xs sm:text-sm px-3 py-2 rounded-full bg-black/30 backdrop-blur-sm ${tag.color} border border-gray-700 flex items-center gap-2`}
                title={tag.name}
              >
                {getTagIcon(tag.name)}
          ...
          <Link to={`/project/${project_id}`} className="block w-full mt-auto">
            <button
              className={`w-full py-3 px-6 rounded-lg font-semibold transition-all duration-300 flex items-center justify-center gap-2 text-[#C3BFFB] bg-transparent hover:text-white`}
            >
              <span>View Details</span>
